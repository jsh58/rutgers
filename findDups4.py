...
# Version 4: checking methylation string matches
...
def getXM(lis):
  for t in lis:
    ...
    if spl[0] == 'XM':
      return spl[-1]
  print 'Error! Cannot find XM in SAM record'
  sys.exit(-1)
    ...
    if chr in pos:
      if loc in pos[chr]:
        if rc in pos[chr][loc]:
          if getXM(spl[11:]) in pos[chr][loc][rc]:
            # location, strand, *and* Me-string match
            dups += 1
          else:
            # everything matches except seq
            pos[chr][loc][rc].append(getXM(spl[11:]))
            fOut.write(line)
            notSeq += 1
        else:
          pos[chr][loc][rc] = [getXM(spl[11:])]
          fOut.write(line)
          uniq += 1
      else:
        pos[chr][loc] = {}

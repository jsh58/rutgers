...
def printOutput(fOut, chrom, meth, minCov, strand, verbose):
  '''
...
def parseSAM(f, bedRegions, bedSites, linkedMeth,
    sortOpt, printed, strand, verbose):
  '''
  ...
  refChrom = ''  # chromsome being analyzed
  for line in f:

    # save SAM header info (incl. chromosome names)
    if line[0] == '@':
      spl = line.rstrip().split('\t')[1].split(':')
      if line[1:3] == 'HD' and spl[0] == 'SO' \
          and spl[1] == 'coordinate':
        coord = True
    ...
    if sortOpt and not coord:
      sys.stderr.write('Error! SAM must be sorted with -b\n')
      sys.exit(-1)
    ...
    # for memory-saving option, produce output and reset
    if sortOpt and chrom != refChrom:
      printed += printOutput(outfile, refChrom, meth[refChrom], \
        minCov, strand, verbose)
      refChrom = chrom
      meth = {}
  ...
  return genome, meth, total, mapped, methCount, count
...
def main():
  ...
  genome, meth, total, mapped, methCount, count = parseSAM(infile,
    bedRegions, bedSites, linkedMeth, sortOpt, outfile, printed,
    strand, verbose)
  if infile != sys.stdin:
    infile.close()
